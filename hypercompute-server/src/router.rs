...
use hypercompute_proto::{
    ...
    Task, TaskStatus, TaskStatusResponse,
};
use std::sync::Arc;
...
async fn submit_task(
    ...
    let resp = SubmitTaskResponse { task_id: task.id, status: task.status.clone() };
...
async fn get_task(
    ...
    // Pending queue.
    {
        ...
        if let Some(task) = q.iter().find(|t| t.id == id) {
            return Ok(Json(TaskStatusResponse { task: task.clone(), result: None, mpi_results: None }));
        }
    }
    // Running.
    ...
    // Completed.
    {
        ...
        if let Some((task, output, mpi)) = done.iter().find(|(t, _, _)| t.id == id) {
            return Ok(Json(TaskStatusResponse {
                task: task.clone(),
                result: output.clone(),
                mpi_results: mpi.clone(),
            }));
        }
    }
...
async fn server_status(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    Json(ServerStatusResponse {
        ...
        completed_tasks: completed, online_nodes: online,
    })
}
